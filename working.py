...
def conv_net(x,keep_prob):
    ...
    out = tf.contrib.layers.fully_connected(inputs=flat, num_outputs=10, activation_fn=tf.nn.softmax)
    return out, l2
...
def main():
    ...
    # Loss and Optimizer
    cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=y)) + (l2[0] + l2[1] + l2[2] + l2[3] + l2[4] + l2[5])
